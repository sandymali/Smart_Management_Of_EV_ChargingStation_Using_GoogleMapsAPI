...
<body>
  ...
  <main id="main">
   ...
   ResultSet rs=st.executeQuery("select * from tbl_slot where slotname='"+search+"'");
   while(rs.next())
   {
	   %>
  	...
  	<td><%=rs.getString(6) %></td>
  	 ...
  	 <TR>
  	<th>View Map</th>
  	<td><a href="map.jsp">View Map</a></td>
  	</TR>
  	
  	 <TR>
  	<th>Booking</th>
  	<td><a href="SearchSlotcontroller?id=<%=rs.getInt(1)%>&slotname=<%=rs.getString(2) %>&address=<%=rs.getString(3) %>&area=<%=rs.getString(4) %>&city=<%=rs.getString(5) %>&state=<%=rs.getString(6) %>"><b>Book</b></a></td>
  	</TR>
